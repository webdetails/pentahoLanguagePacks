...
The Language Pack Installers require Pentaho 11.0+, and depend on the following CTools:
...
There are two ways to get a language pack:

1. **Install via the Pentaho Plugin Manager** (recommended):
   * Open Pentaho, go to **Tools → Plugin Manager**
   * Find the language pack for your locale and install it

2. **Download from GitHub releases**:
   * Download the zip from the [releases page](https://github.com/webdetails/pentahoLanguagePacks/releases)
   * Unzip it to your `pentaho-solutions/system` folder

Alternatively, you can build from source:

```bash
git clone git@github.com:webdetails/pentahoLanguagePacks.git
cd pentahoLanguagePacks
mvn clean package -DlangCode=pt_PT
```

The resulting zip will be in `assemblies/pentaho-plugin/target/dist/`.

Extract the zip to your `pentaho-solutions/system` folder.
